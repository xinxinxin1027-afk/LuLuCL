	...
	Scaffold(
		topBar = {
			TopAppBar(
				...
				title = {
					Text(text = stringResource(R.string.settings), style = h1TextStyle)
				},
				navigationIcon = {
			...
			ModalBottomSheet(
				...
				Box(modifier = Modifier.padding(16.dp)) {
					when (showBottomSheetById) {
						R.string.theme -> ThemeOptionComponent(
							...
							onSelect = { onAction(MainAction.UpdateAppTheme(it)) },
						)

						R.string.language -> LanguageOptionComponent(
							defaultLanguage = appState.language,
						) { onAction(MainAction.UpdateLanguage(it)) }

						R.string.sleep_time -> SleepTimeOptionComponent(
							defaultSleepTime = appState.sleepTime,
						) { onAction(MainAction.UpdateSleepTime(it)) }

						R.string.time_picker -> TimePickerOptionComponent(
							isWheelTimePicker = appState.isWheelTimePicker,
							is24HourTimeFormat = appState.is24hourTimeFormat,
							onSelect = { onAction(MainAction.UpdateTimePicker(it)) },
							onSelectTimeFormat = { onAction(MainAction.UpdateTimeFormat(it)) },
						)

						R.string.swipe_action -> SwipeActionOptionComponent(
							selected = appState.swipeBehaviour,
							onSelect = { onAction(MainAction.UpdateSwipeBehaviour(it)) },
						)

						R.string.sounds -> SoundOptionComponent(
		...
		Column(
			...
			Column(
				...
				SettingsCategoryComponent(categoryTitle = "", categoryList = settingsAbout)
